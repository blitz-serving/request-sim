...
def calculate_throughput_per_millisecond(csv_file):
    ...
    with open(csv_file, 'r') as csvfile:
        ...
        for row in reader:
            s_time = int(float(row['s_time']) + float(row['queue_time'])*1000 + float(row['first_token_time'])*1000)
            e_time = int(row['e_time'])
            ...
            throughput_per_millisecond = output_length / duration_ms
...
def plot_throughput(throughput_per_second, name):
    ...
    plt.title('Decode Throughput Over Time')
    plt.grid(True)
    #plt.show()
